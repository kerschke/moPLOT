...
ggplotPLOTObjSpace = function(obj.space, sinks, height, check.data = TRUE) {
  ...
  g = ggplot() +
    ...
    scale_color_gradientn(colors = gray.colors(500L, start = 0, end = 1, gamma = 0.5, alpha = 1), na.value = "transparent", trans = "log") +
    scale_fill_gradientn(colors = viridisLite::turbo(500L, alpha = 1, begin = 0.05, end = 0.95, direction = 1), trans = "log") +
